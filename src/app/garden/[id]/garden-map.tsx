...
function MapEvents({
  ...
  useEffect(() => {
    function handleCreate(e: L.LeafletEvent) {
      const { shape, layer } = e as L.LeafletEvent & { shape: string; layer: L.Layer };
      const drawType = (map as L.Map & { _pendingShapeType?: ShapeType })._pendingShapeType;
      if (!drawType) return;

      const points =
        shape === "Marker"
          ? [{ lat: (layer as L.Marker).getLatLng().lat, lng: (layer as L.Marker).getLatLng().lng }]
...
export function GardenMap({ garden, shapes }: { garden: Garden; shapes: Shape[] }) {
      ...
      await createShape(garden.id, {
        ...
        heightM:
          pendingShape.type === "house" || pendingShape.type === "tree"
            ...
            : null,
        radiusM: pendingShape.type === "tree" ? Number(radiusInput) || DEFAULT_TREE_RADIUS_M : null,
      });
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[1fr_20rem]">
      <div className="h-[60vh] overflow-hidden rounded-lg border lg:h-[75vh]">
        ...
        >
          <TileLayer
            ...
            maxNativeZoom={19}
          />
          <MapEvents onReady={(map) => (mapRef.current = map)} onCreate={handleCreate} />
      ...
      <div className="space-y-4">
        <Card>
          ...
          <CardContent className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <Button variant="outline" size="sm" onClick={() => startDraw("boundary")}>
                <Square className="h-4 w-4" />
                {t("drawBoundary")}
              </Button>
              <Button variant="outline" size="sm" onClick={() => startDraw("house")}>
                <Home className="h-4 w-4" />
                {t("drawHouse")}
              </Button>
              <Button variant="outline" size="sm" onClick={() => startDraw("tree")}>
                <TreePine className="h-4 w-4" />
                {t("drawTree")}
              </Button>
              <Button variant="outline" size="sm" onClick={() => startDraw("vegetable_plot")}>
            ...
            <div className="border-t pt-2">
              {shapes.length === 0 ? (
                ...
                <ul className="space-y-1">
                  {shapes.map((shape) => (
                    <li key={shape.id} className="flex items-center justify-between text-sm">
        ...
        <Card>
          <CardContent className="space-y-2 pt-4">
            <Button className="w-full" onClick={handleComputeSun} disabled={isComputingSun}>
              {isComputingSun ? (
            ...
            {sunGrid && (
              <>
                <Button variant="ghost" className="w-full" onClick={() => setSunGrid(null)}>
                  {t("hideSun")}
                </Button>
                <p className="text-xs text-muted-foreground">{t("sunHoursLegend")}</p>
                <div
      ...
      <Dialog open={pendingShape !== null} onOpenChange={(next) => !next && setPendingShape(null)}>
        <DialogContent className="sm:max-w-sm">
          <DialogHeader>
            <DialogTitle>
              {pendingShape && t("newShapeTitle", { type: shapeLabels[pendingShape.type] })}
            </DialogTitle>
            <DialogDescription />
          </DialogHeader>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setPendingShape(null)} disabled={isSaving}>
              {t("cancel")}
            </Button>
